...
public:
    template <size_t len> static std::array<float, len> evaluate(double param, std::vector<std::array<float, len>> &coefficients);

    template <size_t len> static std::array<float, len> evaluate_derivative(double param, std::vector<std::array<float, len>> &coefficients);
