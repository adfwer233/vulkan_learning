...
namespace MetaProgramming {
    ...
    namespace details {
        ...
        struct MultiDimensionalArrayImpl{
            using type = std::conditional_t<sizeof...(rest_dim) == 1, std::array<T, dim>, std::array<typename MultiDimensionalArrayImpl<T, rest_dim...>::type, dim>>;
        };

        template<typename T>
        struct MultiDimensionalArrayImpl<T, 0> {
            using type = size_t;
        };
    }
