...
public:
    ...
    PathTracingComputeModel(VklDevice &device, VklScene &scene): device_(device), scene_(scene) {
        ...
        glm::dvec4 *rawTargetImage = new glm::dvec4[n * m];
        for (int i = 0; i < n * m; i++) {
            rawTargetImage[i] = glm::dvec4{1.0, 1.0, 1.0, 1.0};
        }
        std::cout << sizeof(rawTargetImage[0]) << std::endl;
        ...
        auto targetTexture = new VklImage(device_, n, m, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
...
//                                      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        ...
        auto accumulationTexture = new VklImage(device_, n, m, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
