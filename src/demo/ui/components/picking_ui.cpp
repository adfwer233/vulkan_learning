...
void PickingUI::renderImgui() {
    ...
    if (uiManager_.picking_result.has_value()) {
        ...
        if (u < v and u < w) {
            uiManager_.picking_result->vertex_index =
                model_picked->geometry->indices[uiManager_.picking_result->face_index].i;
        } else if (v < u and v < w) {
            uiManager_.picking_result->vertex_index =
                model_picked->geometry->indices[uiManager_.picking_result->face_index].j;
        } else if (w < u and w < v) {
            uiManager_.picking_result->vertex_index =
                model_picked->geometry->indices[uiManager_.picking_result->face_index].k;
        }
