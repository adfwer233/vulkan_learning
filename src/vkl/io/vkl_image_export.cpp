...
void VklImageExporter::exportToImage(VkImage image, uint32_t width, uint32_t height) {
    ...
    device_.createBuffer(width * height * 4, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
    ...
    device_.createImage(
        width, height, VkFormat::VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_LINEAR,
    ...
    vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 0, nullptr, 0,
    ...
    vkCmdCopyImageToBuffer(commandBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, imageBuffer, 1, &copy);

    VkImageCopy imageCopyRegion{.srcSubresource = {.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT, .layerCount = 1},
                                .dstSubresource = {.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT, .layerCount = 1},
                                .extent{.width = width, .height = height, .depth = 1}};

    vkCmdCopyImage(commandBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstImage,
    ...
    assert(vkMapMemory(device_.device(), dstImageMemory, 0, VK_WHOLE_SIZE, 0, (void **)&imageData) == VK_SUCCESS);
    ...
    for (auto i = 0; i < height; i++) {
        for (auto j = 0; j < width; j++) {
            ...
            // pixel.g *= 255.99;
            // pixel.b *= 255.99;
            ss << unsigned(pixel->r) << ' ' << unsigned(pixel->g) << ' ' << unsigned(pixel->b) << ' '
               << unsigned(pixel->a) << '\n';
        }
    }
#endif
